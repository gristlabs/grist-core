...
// Build a probe result through the same roll-up the server probe uses, so the
// stories exercise the real posture/status logic instead of a copy of it.
function makeResult(f: Fixture): BootProbeResult {
  return summarizeOutgoingRequests({
    proxyConfigured: Boolean(f.proxyUntrustedConfigured),
    untrustedDirect: Boolean(f.proxyUntrustedDirect),
    trustedConfigured: Boolean(f.proxyTrustedConfigured),
    requestFunctionEnabled: Boolean(f.requestFunctionEnabled),
    allowedWebhookDomains: f.webhookDomains || [],
    webhookWildcard: Boolean(f.webhookWildcard),
  });
}
