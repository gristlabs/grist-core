...
describe('Sandbox', function () {
  ...
  describe("sandbox restrictions", function () {
    ...
    it("should only have access to directories inside the sandbox root", async function () {
      ...
      let hostDirs = ['', 'imports', 'functions', 'imports/fixtures'];
      if (process.env.GRIST_RUNNING_UNDER_MACOS !== '1') {
        hostDirs = hostDirs.concat(['__pycache__', 'imports/__pycache__', 'functions/__pycache__']);
      }
      const expectedDirs = hostDirs.map(dir => path.join(sandboxRoot, dir));
    ...
    it("gvisor should have no write access to sandbox files", async function () {
      if (sandbox.getFlavor() !== 'sandboxed') {
        this.skip();
      }
      const sandboxRoot = await sandbox.pyCall('test_get_sandbox_root');
