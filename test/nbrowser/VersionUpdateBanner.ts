...
describe('VersionUpdateBanner', function() {
  ...
  before(async function() {
    ...
    // The fake server sets its payload after it has served a version
    // at least once.
    assert.isNull(fakeServer.payload);
    await server.restart(true);
    // Let's check it really was called now.
    assert.isNotNull(fakeServer.payload);
  });
