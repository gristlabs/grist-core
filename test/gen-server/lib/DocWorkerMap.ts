...
describe('RedisClient', function() {
  it('has working promise-based interface', async function() {
    if (!process.env.TEST_REDIS_URL) { this.skip(); }
    const cli = createClient(process.env.TEST_REDIS_URL);
    await cli.delAsync('test');
    assert.equal(await cli.getAsync('test'), null);
    await cli.setAsync('test', 'hello');
    assert.equal(await cli.getAsync('test'), 'hello');
    await cli.quitAsync();
  });
});
