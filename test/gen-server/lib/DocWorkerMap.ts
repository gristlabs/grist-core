...
describe('DocWorkerMap', function() {
  ...
  describe('load', function() {
    ...
    before(async function() {
      ...
      api = await session.createHomeApi('chimpy', 'nasa');
    ...
    it('initializes when worker is added', async function() {
      const availableWorkerIds = await cli.smembersAsync('workers-available-default');
      assert.lengthOf(availableWorkerIds, 3);
      for (const id of availableWorkerIds) {
        const load = await cli.hgetallAsync(`worker-${id}-load`);
        assert.hasAllKeys(load, [
          'freeMemoryMB',
          'totalMemoryMB',
          'totalAssignmentsCount',
          'loadingDocsCount',
        ]);
        assert.isAbove(Number(load!.totalMemoryMB), 0);
        assert.isBelow(Number(load!.freeMemoryMB), Number(load!.totalMemoryMB));
        assert.equal(Number(load!.totalAssignmentsCount), 0);
        assert.equal(Number(load!.loadingDocsCount), 0);
      }
    });

    it('updates when worker is assigned or released', async function() {
      await workers.assignDocWorker(docId);
      const workerId = (await workers.getDocWorker(docId))?.docWorker.id;
      let load = await cli.hgetallAsync(`worker-${workerId}-load`);
      let loadNewAssignments = await cli.smembersAsync(`worker-${workerId}-load-new-assignments`);
      assert.equal(Number(load!.totalAssignmentsCount), 1);
      assert.equal(Number(load!.loadingDocsCount), 0);
      assert.deepEqual(loadNewAssignments, [docId]);

      await workers.releaseAssignment(workerId!, docId);
      load = await cli.hgetallAsync(`worker-${workerId}-load`);
      loadNewAssignments = await cli.smembersAsync(`worker-${workerId}-load-new-assignments`);
      assert.equal(Number(load!.totalAssignmentsCount), 0);
      assert.isEmpty(loadNewAssignments);
    });

    it('updates when document is opened', async function() {
      const updateWorkerLoad = sandbox!.spy(DocWorkerMap.prototype, "updateWorkerLoad");

      await api.getDocAPI(docId).getRows('Table1');
      const workerId = (await workers.getDocWorker(docId))?.docWorker.id;
      const load = await cli.hgetallAsync(`worker-${workerId}-load`);
      const loadNewAssignments = await cli.smembersAsync(`worker-${workerId}-load-new-assignments`);
      assert.equal(Number(load!.totalAssignmentsCount), 1);
      assert.equal(Number(load!.loadingDocsCount), 0);
      assert.isEmpty(loadNewAssignments);
      assert.isTrue(updateWorkerLoad.calledTwice);
      assert.equal(updateWorkerLoad.firstCall.args[0], workerId);
      assert.isAbove(updateWorkerLoad.firstCall.args[1].freeMemoryMB!, 0);
      assert.deepInclude(updateWorkerLoad.firstCall.args[1], {
        loadingDocsCountDelta: 1,
        loadingDocIds: [docId],
      });
      assert.equal(updateWorkerLoad.secondCall.args[0], workerId);
      assert.isAbove(updateWorkerLoad.secondCall.args[1].freeMemoryMB!, 0);
      assert.deepInclude(updateWorkerLoad.secondCall.args[1], {
        loadingDocsCountDelta: -1,
        loadingDocIds: [],
      });
      await workers.releaseAssignment(workerId!, docId);
    ...
    it('updates when worker is removed and re-added', async function() {
      for (const mode of ['clean', 'crash'] as const) {
        ...
        const worker = servers[workerId!];
        await worker.stopListening(mode);
        await worker.restartListening();
        load = await cli.hgetallAsync(`worker-${workerId}-load`);
        loadNewAssignments = await cli.smembersAsync(`worker-${workerId}-load-new-assignments`);
        assert.equal(Number(load!.totalAssignmentsCount), 0);
        assert.isEmpty(loadNewAssignments);
