...
describe('ApiServer', function() {
  ...
  describe('DocApiKey Endpoint', function(){
    ...
    afterEach(async function() {
      oldEnv.restore();
      const did = await dbManager.testGetId('Curiosity');
      await dbManager.deleteDocApiKeys(did as string);
    });
    ...
    it('POST /api/docs/{did}/apikey status 400 when linkId already exists for given :did', async function() {
      ...
      const resp1 = await axios.post(`${homeUrl}/api/docs/${did}/apikey`, body, chimpy);
      // Assert that docApiKey is created
      assert.equal(resp1.status, 200);

      const resp2 = await axios.post(`${homeUrl}/api/docs/${did}/apikey`, body, chimpy);
      // Assert that docApiKey with same LinKid can't be created
    ...
    it('PATCH /api/docs/{did}/apikey/{linkId} returns 200 on options.access modification', async function() {
      ...
      const patchResp = await axios.patch(`${homeUrl}/api/docs/${did}/apikey/${linkId}`,
        {options: JSON.stringify(newOptions)}, chimpy);
    ...
    it('GET /api/docs/{did}/apikeys is operational', async function() {
      ...
      assert.equal(respPost2.status, 200, "Second docApiKeyCreated");

      // Get doc-api-keys
      const resp = await axios.get(`${homeUrl}/api/docs/${did}/apikeys`, chimpy);
      // Assert that endpoint reponds with SUCCESS
      assert.equal(resp.status, 200, "Get docApiKeys");

      // Assert there's two doc api keys for this 'Curiosity'
