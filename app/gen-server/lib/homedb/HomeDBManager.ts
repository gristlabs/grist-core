...
export class HomeDBManager {
  ...
  public async getDoc(reqOrScope: Request | Scope, transaction?: EntityManager): Promise<Document> {
    ...
    const docRemoved = doc.removedAt || doc.workspace.removedAt;
    const onlyShowRemoved = !scope.showAll && scope.showRemoved;
    // if onlyShowRemoved XOR docRemoved, then we have no document
    // (the condition inside the "if" below is an XOR).
    if ((!onlyShowRemoved && docRemoved) ||(onlyShowRemoved && !docRemoved) ) {
