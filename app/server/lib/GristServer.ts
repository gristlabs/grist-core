...
import {
  ...
  IAuditLogger,
} from "app/server/lib/IAuditLogger";
...
export interface GristServer extends StorageCoordinator {
  ...
  setLatestVersionAvailable(
    latestVersionAvailable: LatestVersionAvailable
  ): void;
  publishLatestVersionAvailable(
    latestVersionAvailable: LatestVersionAvailable
  ): Promise<void>;
  getApiRequestBodyLimit(): string;
...
export function createDummyGristServer(): GristServer {
  return {
    ...
    getHomeUrlByDocId() {
      return Promise.resolve("http://localhost:4242");
    },
    getMergedOrgUrl() {
      return "http://localhost:4242";
    },
    ...
    getWidgetRepository() {
      throw new Error("no widget repository");
    },
    getNotifier() {
    ...
    getPubSubManager(): IPubSubManager {
      throw new Error("no PubSubManager");
    },
    hasNotifier() {
      return false;
    },
    ...
    getJobs(): GristJobs {
      throw new Error("no job system");
    },
    getBilling() {
      throw new Error("no billing");
    },
    getDoomTool() {
      throw new Error("no doom tool");
    },
    getLatestVersionAvailable() {
      throw new Error("no version checking");
    },
    setLatestVersionAvailable() {
      /* do nothing */
    },
