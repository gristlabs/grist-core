...
const translate = (req: express.Request, key: string, args?: any) =>
  req.t(`sendAppPage.${key}`, args)?.toString();

const GRIST_FEATURE_FORM_FRAMING = appSettings
  .section("features")
  .flag("formFraming")
  .requireString({
...
/**
 * Get the maximum API request body size in bytes using appSettings.
 * Returns undefined if not set to maintain the same behavior as before.
 */
function getMaxApiRequestBodySize(): number | undefined {
  ...
  return limitMB ? limitMB * 1024 * 1024 : undefined;
}
...
export function makeGristConfig(
  ...
  const pathOnly =
    ...
    (homeUrl && new URL(homeUrl).hostname === "localhost") ||
    false;
  ...
  return {
    ...
    maxUploadSizeAttachment:
      ...
      undefined,
    maxApiRequestBodySize: getMaxApiRequestBodySize(),
    ...
    automaticVersionCheckingAllowed: isAffirmative(
      process.env.GRIST_ALLOW_AUTOMATIC_VERSION_CHECKING
    ),
    ...
    forceEnableEnterprise: isAffirmative(
      process.env.GRIST_FORCE_ENABLE_ENTERPRISE
    ),
 ...
 */
export function makeSendAppPage({
  server,
  staticDir,
  tag,
  testLogin,
  ...
  return async (
    ...
    const content = fileContent
      ...
      .replace(
        "<!-- INSERT TITLE -->",
        getDocName(config) ?? escapeExpression(translate(req, "Loading..."))
      )
 ...
 * Note: The string returned is escaped and safe to insert into HTML.
 */
function getPageMetadataHtmlSnippet(
  req: express.Request,
  config: GristLoadConfig
): string {
  ...
  const openGraphPreviewImage =
    process.env.GRIST_OPEN_GRAPH_PREVIEW_IMAGE ||
    commonUrls.openGraphPreviewImage;
  const image = escapeExpression(
    maybeDoc?.options?.icon ?? openGraphPreviewImage
  );
  ...
  const title = maybeDocTitle
    ? maybeDocTitle + getPageTitleSuffix(config)
    : translate(req, "og-title");
