...
export class DocWorkerApi {
  ...
  public addEndpoints() {
    ...
    this._app.get("/api/docs/:docId/tables", canView,
      withDoc(async (activeDoc, req, res) => {
        const expandParam = optStringParam(req.query.expand, "expand");
        const expand = (expandParam?.split(",") ?? []).map(object =>
          ExpandableTableObject.check(object));
        const tables = await handleSandboxError("", [],
          activeDoc.getTables(docSessionFromRequest(req), expand));
        res.json({ tables });
      }),
    );
