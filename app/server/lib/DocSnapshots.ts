...
export class DocSnapshotInventory implements IInventory {
    ...
    public async uploadAndAdd(key: string,
        ...
        await this._mutex.runExclusive(key, async () => {
            ...
            if (snapshots[0] && snapshots[0].snapshotId === snapshot.snapshotId) {
                return;
            }
...
export function shouldKeepSnapshots(snapshots: ObjSnapshotWithMetadata[], snapshotWindow?: SnapshotWindow): boolean[] {
    ...
    const capObjectString = process.env.SNAPSHOT_TIMEBUCKET_CAP || '{"hour": "25", "day": "32", "isoWeek": "12", "month": "96", "year": "1000"}';
...
// has been saved based on a corresponding rule.
    ...
    return snapshots.map((snapshot, index) => {
        ...
        let keep = index < (process.env.NUM_SNAPSHOT_KEEP || 5);   // Keep 5 most recent versions if NUM_SNAPSHOT_KEEP not exist

        for (const bucket of buckets) {
            if (updateAndCheckRange(date, bucket)) {
                keep = true;
            }
        }
