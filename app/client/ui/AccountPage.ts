...
import { reportSuccess } from "../models/errors";
...
designed to ensure that you're the only person who can access your account, even if someone knows your password."),
  ...
  */
  ...
  private async _setGravatarPicture() {
    const user = this._userObs.get(); 
    if (!user?.email) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        reportError(new Error(`Gravatar image not found for email: ${user.email}`));
        return;
      }
      await this._appModel.api.updateUserPicture(gravatarUrl);
      await this._fetchAll();
    } catch (e) {
      reportError(new Error(`Failed to fetch Gravatar image for email: ${user.email}. Error: ${e}`));
    }
  }
  ...
  private async _applyGravatarWithLoader() {
    ...
    try {
      await this._setGravatarPicture();
