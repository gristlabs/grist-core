...
| GRIST_ENABLE_SERVICE_ACCOUNTS | enables the `service accounts` feature. This feature add the ability for the users to create other special service users that they can manage, and to whom they can give retricted access to choosen ressources. Usefull for users using third parties automation tools and who don't want to expose their own Api Key anymore. Unset by default |
